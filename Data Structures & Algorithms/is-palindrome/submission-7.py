class Solution:
    def isPalindrome(self, s: str) -> bool:
        # convert string to all lowercase alphanumeric chars
        # two pointer to check if string reversed 
        
        # edge case: a str with <1 or 0 char is a palindrome
        if len(s) <= 1:
            return True
        ...
        while l < r:
            # clean the string by skipping all alnum chars
            ...
            # if the l and r char are diff, return false 
            if s[l].lower() != s[r].lower(): 
                return False
            # move onto the next set of chars, L and R pair
            l += 1
            r -= 1
        return True
