class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        # edge case, an str is not an anagram if it has a different number of characters
        # if there's no anagram, then return that same word in a sublist
        # mapping each string as the key to a list of anagrams
...
# i'm going to store each anagram "key" through a ascii value map to avoid sorting
# this way, i can create keys for each anagram based on which letters there are
# if two words 
